...
function createBloodPoolTexture() {
    ...
    function drawBlob(cx, cy, r, alpha) {
        ...
        for (let i = 1; i < points.length; i++) {
            ...
            ctx.quadraticCurveTo(prev.x, prev.y, cx2, cy2);
        }
        ctx.closePath();
        const grad = ctx.createRadialGradient(cx, cy, 0, cx, cy, r);
...
export function generateMap4(scene, size, wallHeight, tileSize) {
    ...
    for (let y = 0; y < size; y++) {
        for (let x = 0; x < size; x++) {
            if (distFromExit[y][x] !== Infinity && distFromExit[y][x] > maxDistGlobal) maxDistGlobal = distFromExit[y][x];
        }
    }
    if (maxDistGlobal === 0) maxDistGlobal = 1;

    const wallTexs = [createLabWallTexture(), createLabWallTexture(), createLabWallTexture()];
    const floorTex = createLabFloorTexture();
    ...
    function isRoomHorizontalWall(x, y) {
        if (x < ROOM_MIN || x > ROOM_MAX) return false;
        return (y === ROOM_MIN || y === ROOM_MAX + 1);
    }
    function isRoomVerticalWall(x, y) {
    ...
    for (let y = 0; y < size; y++) {
        for (let x = 0; x <= size; x++) {
            ...
            if (!hasWall) continue;
            if (isRoomVerticalWall(x, y)) continue;
...

  {
    ...
    function addRail(x1, z1, x2, z2) {
        ...
        m.position.set((x1 + x2) / 2, wallHeight - 0.08, (z1 + z2) / 2);
    ...
    const tableMat = new THREE.MeshStandardMaterial({ color: 0x2a2e33, roughness: 0.55, metalness: 0.7 });
    ...
    const ivPole = new THREE.Mesh(new THREE.CylinderGeometry(0.03, 0.03, 1.8, 6), tableMat);
    ivPole.position.y = 0.9;
    ivPole.castShadow = true;
    ivGroup.add(ivPole);
    const ivBase = new THREE.Mesh(new THREE.CylinderGeometry(0.25, 0.25, 0.04, 8), tableMat);
    ...
    for (const p of propList) {
        if (p.type === 'table') {
            ...
            g.add(pole);
            const screenBody = new THREE.Mesh(new THREE.BoxGeometry(0.7, 0.45, 0.04),
                new THREE.MeshStandardMaterial({ color: 0x0a0c0e, roughness: 0.5, metalness: 0.4 }));
            screenBody.position.y = 0.45;
            screenBody.castShadow = true;
            g.add(screenBody);
            const screenGlow = new THREE.Mesh(new THREE.PlaneGeometry(0.62, 0.38),
                new THREE.MeshBasicMaterial({ color: 0x223344 }));
            screenGlow.position.set(0, 0.45, 0.021);
            g.add(screenGlow);
            g.position.set(p.x, 0, p.z);
            g.rotation.y = Math.random() * Math.PI * 2;
            group.add(g);
